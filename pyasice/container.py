...
from xml.etree.ElementTree import Element, parse as xml_parse, tostring as xml_tostring
...
class Container(object):
    ...
    MANIFEST_PATH = f'{META_DIR}/{MANIFEST_FILE}'
    MANIFEST_NS = 'urn:oasis:names:tc:opendocument:xmlns:manifest:1.0'
    MANIFEST_NAMESPACES = {
        'manifest': MANIFEST_NS,
    }
    MANIFEST_TAG_FILE_ENTRY = f'{MANIFEST_NS}file-entry'
    MANIFEST_ATTR_MEDIA_TYPE = f'{MANIFEST_NS}media-type'
    MANIFEST_ATTR_FULL_PATH = f'{MANIFEST_NS}full-path'
    ...
    def add_file(self, file_name: str, binary_data: bytes, mime_type='application/octet-stream', compress=True):
        ...
        new_manifest_entry = Element(self.MANIFEST_TAG_FILE_ENTRY)
    ...
    def _write_manifest(self):
        ...
        with self.zip_writer as zip_file:
            zip_file.writestr(
                self.MANIFEST_PATH, b'<?xml version="1.0" encoding="UTF-8"?>' + xml_tostring(manifest_xml)
            )
    ...
    def _get_manifest_xml(self):
        if self._manifest is None:
            ...
            with open(self.MANIFEST_TEMPLATE_FILE, "rb") as f:
                self._manifest = xml_parse(f.read())
    ...
    def verify_container(self):
        ...
        if failed:
            raise self.Error(f'The container contains errors. First broken file: {failed}')
...
class CAdES(Container):
    """
    Manipulate ASiC-E CAdES additional container

    Technical Specification:
    - CAdES: ETSI TS 103 173 V2.2.1
    - ASiC-E: ETSI TS 103 174 V2.2.1
    """
    SIGNATURE_FILES_REGEX = r'^%s/signature([a-f0-9\-]+)\.p7s$' % Container.META_DIR
...
class XAdES(Container):
    ...
    SIGNATURE_FILES_REGEX = r'^%s/signatures(\d+)\.xml$' % Container.META_DIR
    SIGNATURE_FILES_TEMPLATE = '%s/signatures{}.xml' % Container.META_DIR
